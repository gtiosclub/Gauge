...
struct VotesTabView: View {
    ...
    func fetchRespondedPosts() {
        ...
        for id in ids {
            db.collection("POSTS").document(id).getDocument { doc, error in
                ...
                if let type = data["type"] as? String, type == PostType.BinaryPost.rawValue {
                    ...
                    // Responses
                    group.enter()
                    db.collection("POSTS").document(id).collection("RESPONSES").getDocuments { snapshot, _ in
                    ...
                    // Comments
                    group.enter()
                    db.collection("POSTS").document(id).collection("COMMENTS").getDocuments { snapshot, _ in
                        if let docs = snapshot?.documents {
                    ...
                    // Views
                    group.enter()
                    db.collection("POSTS").document(id).collection("VIEWS").getDocuments { snapshot, _ in
