    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 16) {
                ForEach(postVM.allQueriedPosts.filter { $0.userId == userVM.user.userId }, id: \.postId) { post in
                    if let binary = post as? BinaryPost {
                        Button(action: {
                            selectedPost = binary
                        }) {
                            TakeCard(
                                ...
                                votes: binary.responseResult1 + binary.responseResult2,
